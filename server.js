...
function buildInitialState(levelIdx) {
  // Levels mirror the client config
  const LEVELS = [
    { cols:28, rows:46, numColors:7, maxMoves:30 },
    { cols:30, rows:50, numColors:7, maxMoves:33 },
    { cols:32, rows:52, numColors:7, maxMoves:36 },
    { cols:34, rows:54, numColors:7, maxMoves:38 },
    { cols:36, rows:56, numColors:7, maxMoves:40 },
    { cols:38, rows:58, numColors:7, maxMoves:42 },
    { cols:40, rows:60, numColors:7, maxMoves:44 },
    { cols:42, rows:62, numColors:7, maxMoves:46 },
  ];
  const cfg = LEVELS[Math.min(levelIdx, LEVELS.length - 1)];
  const { cols, rows, numColors, maxMoves } = cfg;
  ...
  // Spawn positions — P1 bottom area, P2 top area
  function randInt(lo, hi) { return lo + Math.floor(Math.random() * (hi - lo + 1)); }
  const padC = Math.max(2, Math.floor(cols * 0.08));
  const padR = Math.max(2, Math.floor(rows * 0.08));

  let p1r, p1c, p2r, p2c, attempts = 0;
  do {
    p1r = randInt(Math.floor(rows * 0.50), Math.floor(rows * 0.70));
    p1c = randInt(padC, cols - 1 - padC);
    p2r = randInt(Math.floor(rows * 0.25), Math.floor(rows * 0.45));
    p2c = randInt(padC, cols - 1 - padC);
    attempts++;
  } while (Math.sqrt((p1r-p2r)**2+(p1c-p2c)**2) < Math.min(cols,rows)*0.25 && attempts < 300);
...
io.on('connection', socket => {
  ...
  // Create room
  socket.on('create_room', ({ levelIdx = 0, endless = false }) => {
    let code;
  ...
  socket.on('join_room', ({ code }) => {
    ...
    room.state = buildInitialState(room.levelIdx);
    room.state.endless = room.endless;
