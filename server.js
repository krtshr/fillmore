...
function buildInitialState(levelIdx, clientMaxMoves, endless) {
  ...
  // Build balanced grid — each half has equal color distribution
  ...
  const grid = [];
  const baseHalf = buildBalancedHalf(halfRows, cols, numColors);

  // Random color permutation for second half — identical groups, different colors
  function randomColorRemap(n) {
  ...
  let remap = randomColorRemap(numColors);
  let att = 0;
  while (remap.every((v,i) => v===i) && att++<10) remap = randomColorRemap(numColors);

  const otherHalf = baseHalf.map(row => row.map(c => remap[c]));

  const topRows = rows - halfRows;
  for (let r = 0; r < topRows; r++) {
    grid[r] = [];
    const mirrorR = topRows - 1 - r;
    const srcRow = mirrorR < otherHalf.length ? otherHalf[mirrorR] : otherHalf[0];
    for (let c = 0; c < cols; c++) {
      grid[r][c] = srcRow[cols - 1 - c];
    }
  }
  for (let r = 0; r < halfRows; r++) grid[topRows + r] = baseHalf[r];
  ...
  // Balance spawn zones — ensure all colors appear within radius 4 of each spawn
  function balanceSpawnZone(homeR, homeC, radius) {
    const zone = [];
    for (let r = Math.max(0, homeR-radius); r <= Math.min(rows-1, homeR+radius); r++) {
      for (let c = Math.max(0, homeC-radius); c <= Math.min(cols-1, homeC+radius); c++) {
        if (Math.abs(r-homeR) + Math.abs(c-homeC) <= radius) zone.push([r,c]);
      }
    }
    const present = new Set(zone.map(([r,c]) => grid[r][c]));
    const missing = [];
    for (let ci = 0; ci < numColors; ci++) if (!present.has(ci)) missing.push(ci);
    if (!missing.length) return;
    const colorCount = {};
    zone.forEach(([r,c]) => { const col = grid[r][c]; colorCount[col] = (colorCount[col]||0)+1; });
    const sorted = zone.slice().sort(([r1,c1],[r2,c2]) => (colorCount[grid[r2][c2]]||0) - (colorCount[grid[r1][c1]]||0));
    let mi = 0;
    for (const [r,c] of sorted) {
      if (mi >= missing.length) break;
      const col = grid[r][c];
      if (colorCount[col] > 1 && !(r===homeR && c===homeC)) {
        colorCount[col]--;
        grid[r][c] = missing[mi++];
      }
    }
  }
  balanceSpawnZone(p1r, p1c, 4);
  balanceSpawnZone(p2r, p2c, 4);
