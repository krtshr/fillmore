...
function buildInitialState(levelIdx, clientMaxMoves, endless) {
  ...
  const topHalf    = buildBalancedHalf(rows - halfRows, cols, numColors);
  const bottomHalf = buildBalancedHalf(halfRows, cols, numColors);

  const grid = [];
  for (let r = 0; r < rows - halfRows; r++) grid[r] = topHalf[r];
  for (let r = 0; r < halfRows; r++) grid[rows - halfRows + r] = bottomHalf[r];
